...
counts_gather <- gather(counts_df,
                        ...
                        value = 'count',
                        na.rm = TRUE)
# see the fill operation to replace NA values with 0
...
DM_animals <- filter(animals,
                     species_id == 'DM')
DM_animals <- group_by(DM_animals, month)
sol3 <- summarize(DM_animals, avg_weight = mean(weight, na.rm = TRUE), avg_hf = mean(hindfoot_length, na.rm = TRUE))
...
## see also summarize each & mutate each
